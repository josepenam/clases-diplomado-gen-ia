...
- Las ventas de punto de venta (`trx_pos.mnt`) están en pesos CHILENOS, pero
  las reservas de agencias (`res_agt.imp`) están en pesos ARGENTINOS — montos
  del mismo orden de magnitud, y NADA en el esquema lo dice: la moneda es
  conocimiento tribal. Esa es la trampa.
...
-- reservas de agencias (¡en pesos argentinos!) ---------------------------
...
CREATE TABLE res_agt (
    ...
    imp     REAL NOT NULL             -- 'importe'... en ARS, pero eso no está escrito en ninguna parte
);
...
CREATE TABLE tc_d (
    f   TEXT PRIMARY KEY,
    vlr REAL NOT NULL                -- ¿tipo de cambio de qué a qué? tampoco lo dice
);
"""
...
AGENCIAS = [
    (1, "Nieve Argentina Viajes", "AR"),
    (2, "Bariloche Ski Tours", "AR"),
    (3, "Mendoza Andes Travel", "AR"),
    (4, "Cono Sur Operadores", "AR"),
]
...
def crear(ruta: Path = RUTA_DB, verboso: bool = True) -> Path:
    ...
    # tipo de cambio ARS→CLP: camina alrededor de 0,75 CLP por 1 ARS
    fx, valor = [], 0.75
    for d in range(DIAS_TEMPORADA):
        valor = max(0.68, min(0.82, valor + rng.uniform(-0.006, 0.006)))
        fx.append(((INICIO_TEMPORADA + timedelta(days=d)).isoformat(), round(valor, 4)))
    con.executemany("INSERT INTO tc_d VALUES (?,?)", fx)
    ...
    # reservas de agencias, EN PESOS ARGENTINOS: paquetes por persona.
    # El monto por pax (~150k-380k ARS) equivale a 110k-280k CLP: el mismo
    # orden de magnitud que una venta POS — la moneda no se delata por el tamaño.
    reservas = []
    for res_id in range(1, 401):
        ...
        reservas.append((res_id, f_res.isoformat(), rng.randint(1, 4), pax, round(pax * rng.uniform(150_000, 380_000), 2)))
